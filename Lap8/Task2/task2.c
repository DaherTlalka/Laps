...
/*---------------------------------------------------------------------------*\
| Function: Print_Symbols::ELF.h
| Purpose: printing the symbol tabel
|
| Input:
|         state *st: state of the file
|
| Output: the value , index,syction name andsymbol name of the symbol tabel.
| Return: VOID.
\*---------------------------------------------------------------------------*/
...
void Print_Symbols(state *st)
{
    ...
    st->start_map = mmap(0, st->fd_stat.st_size, PROT_READ, MAP_PRIVATE, st->fd, 0);
    Elf32_Ehdr *ehd = (Elf32_Ehdr *)st->start_map;                  // header
    Elf32_Shdr *shd = (Elf32_Shdr *)(st->start_map + ehd->e_shoff); // section
    Elf32_Shdr *sh_strtab = &shd[ehd->e_shstrndx];
    char *strtab;
    Elf32_Sym *symtab;
    char *pointerTab = st->start_map + sh_strtab->sh_offset;
    if (debu)
    {
        printf("offset: ");
        printf("Symbol table '%s' contains %d enteries:\n", pointerTab + shd->sh_name, (shd->sh_size) / (shd->sh_entsize));
    }
    int numsymb;
    for (int i = 0; i < ehd->e_shnum; i++)
    {
        if (shd[i].sh_type == SHT_STRTAB)
        {
            strtab = (char *)st->start_map + shd[i].sh_offset; // address
        }
    }
    printf("[Nr]       Value    Section_index     Section_name\t                     Symbol_name:\n");
    for (int i = 0; i < ehd->e_shnum; i++)
    {
        if (shd[i].sh_type == SHT_SYMTAB)
        {
            symtab = st->start_map + shd[i].sh_offset;
            numsymb = shd[i].sh_size / shd[i].sh_entsize;

            for (int j = 0; j < numsymb; j++)
            {
                printf("[%2d]\t%08x\t", j, symtab[j].st_value);
                if (symtab[j].st_shndx != SHN_UNDEF && symtab[j].st_shndx != SHN_COMMON && symtab[j].st_shndx != SHN_ABS)
                    printf("%5d\t\t%-16s\t ", symtab[j].st_shndx, &pointerTab[shd[symtab[j].st_shndx].sh_name]);
