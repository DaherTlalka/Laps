...
void kill_virus(char *fileName, int signatureOffset, int signatureSize){
    ...
    fseek(infect,signatureOffset,SEEK_SET);
    unsigned char loc = 0x90;
    int smallthen=0;
    for(;smallthen<signatureSize;smallthen++){
        fwrite(&loc,1,1,infect);
    }
    fclose(infect);
}
void Load_signatures(FILE* input){
...
void Fix_file(FILE* Infected){
    ...
    char location[20];
    scanf("%s",location);
    numOfBytes = atoi(location);
    printf("Enter the size of the virus signature: \n");
    scanf("%s",location);
    sign = atoi(location);
    kill_virus(fileName,numOfBytes,sign);
}
